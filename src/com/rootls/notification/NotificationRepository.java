...
//            timer.purge();
    ...
    public void addAppleDevice(User user) {
        NotificationSmsPool.IOSDevice device = new NotificationSmsPool.IOSDevice(user.getImei(), user.getUsername());
...
//        JdbcTemplate jdbcTemplate = new JdbcTemplate(BasicDataSource.class.cast(getCurrentWebApplicationContext().getBean("dataSource")));
        ...
        List<NotificationSmsPool.IOSDevice> devices = getJdbcTemplate().query(sql, new RowMapper<NotificationSmsPool.IOSDevice>() {
            @Override
            public NotificationSmsPool.IOSDevice mapRow(ResultSet rs, int i) throws SQLException {
                return new NotificationSmsPool.IOSDevice(
                        rs.getString("user_deviceToken"),
                        rs.getString("user_mobile")
                );
            }
        });
        Map<String, NotificationSmsPool.IOSDevice> mobileDeviceMap = new HashMap<String, NotificationSmsPool.IOSDevice>();
        for (NotificationSmsPool.IOSDevice device : devices) {
    ...
    public synchronized void pushNotification() {
        try {
            ...
            for (NotificationSmsPool.IOSDevice iosDevice : notifyPool.getMobileIOSDeviceMap().values()) {
